...
async function runDemo() {
  ...
  try {
    ...
    console.log(">>> Tip: Ensure MongoDB Atlas cluster is reachable or local mongod is running.")
    process.exit(1)
  }
  ...
  try {
    ...
    console.log(">>> Reset test collections: departments, doctors, appointments.")
  } catch (e) {
  ...
  console.log(`[+] 3.1 insertMany(): Inserted ${deptResult.insertedCount} Departments`)
  ...
  console.log(`[+] 3.2 insertMany(): Inserted ${docResult.insertedCount} Doctors`)
  ...
  console.log(`[+] 3.3 insertMany(): Inserted ${apptResult.insertedCount} Appointments`)

  console.log("\n[4] CRUD OPERATIONS: READ / QUERY")
  console.log("---------------------------------------------------------------")
  console.log("Q1: Query Doctors with experience >= 10 years (Filtering & Projection):")
  const experiencedDocs = await db.collection("doctors").find(
    { experience: { $gte: 10 } },
  ...
  const cardAppts = await db.collection("appointments").find(
    { department: "Cardiology", status: "confirmed" },
    { projection: { appointmentId: 1, patientName: 1, doctorName: 1, date: 1, time: 1, _id: 0 } }
  ).toArray()
  ...
  console.log(`[+] updateOne(): Updated rating and incremented review count for Dr. Amit Sharma (Modified: ${updateRes.modifiedCount})`)

  console.log("\n[6] CRUD OPERATIONS: DELETE")
  console.log("---------------------------------------------------------------")
  const deleteRes = await db.collection("appointments").deleteOne({ appointmentId: "APT-2026-104" })
  console.log(`[+] deleteOne(): Deleted cancelled appointment APT-2026-104 (Deleted: ${deleteRes.deletedCount})`)

  console.log("\n[7] MONGODB AGGREGATION COMMANDS")
  console.log("---------------------------------------------------------------")
  console.log("Pipeline 1: Total Appointments and Revenue Grouped by Department ($group & $sort):")
  const aggDept = await db.collection("appointments").aggregate([
    {
      $group: {
        _id: "$department",
  ...
  const aggJoin = await db.collection("appointments").aggregate([
    {
      $lookup: {
        from: "doctors",
        localField: "doctorId",
  ...
  const aggDoctorSummary = await db.collection("appointments").aggregate([
    {
      $group: {
        _id: "$doctorName",
        totalPatients: { $sum: 1 },
