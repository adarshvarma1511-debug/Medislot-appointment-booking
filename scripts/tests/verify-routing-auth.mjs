...
async function runTests() {
  ...
  console.log(
    "TEST 1: Checking public Home page ('/') without authentication...",
  )
  ...
  assert(
    homeHtml.includes("Book Your Doctor"),
    "Home page contains 'Book Your Doctor'",
  )

  assert(homeHtml.includes("How It Works"), "Home page contains 'How It Works'")

  assert(
    !homeHtml.includes("Doctor's Appointment Queue"),
    "Home page does NOT contain Doctor Dashboard queue",
  )

  assert(
    !homeHtml.includes("Doctor Portal"),
    "Home page does NOT contain Doctor Portal sidebar",
  )

  console.log(
    "✓ TEST 1 PASSED: Home page ('/') correctly renders the public MediSlot landing page.\n",
  )
  ...
  assert(
    setCookie && setCookie.includes("medislot_token="),
    "medislot_token cookie was set",
  )

  console.log(
    "✓ TEST 2 PASSED: Doctor authenticated and session token issued.\n",
  )
  ...
  assert.strictEqual(
    logoutData.success,
    true,
    "Logout endpoint reports success",
  )

  const logoutCookies = logoutRes.headers.get("set-cookie") || ""

  assert(
    logoutCookies.includes("medislot_token=;") ||
  ...
  console.log(
    "✓ TEST 4 PASSED: Logout endpoint successfully cleared session cookie.\n",
  )

  // TEST 5: Verify /api/auth/me fails after logout (unauthenticated)

  console.log(
    "TEST 5: Verify /api/auth/me fails without token (or with cleared cookie)...",
  )
  ...
  assert.strictEqual(
    unauthMeRes.status,
    401,
    "/api/auth/me returns 401 Unauthorized without session",
  )
  ...
  console.log(
    "✓ TEST 5 PASSED: User is completely unauthenticated after logout.\n",
  )

  // TEST 6: Verify Doctor Dashboard route is protected

  console.log(
    "TEST 6: Checking that /doctor/dashboard requires authorization...",
  )
  ...
  assert(
    docDashHtml.includes("Verifying Doctor Authorization") ||
      docDashHtml.includes("animate-spin"),

    "Doctor Dashboard does not render unprotected content to unauthenticated requests",
  )

  console.log(
    "✓ TEST 6 PASSED: Protected route guards unauthenticated access.\n",
  )
