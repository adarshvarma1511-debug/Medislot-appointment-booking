...
async function loginUser(email, password, role) {
  ...
  assert.strictEqual(
    res.status,
    200,
    `Login failed for ${email}: ${data.error || res.statusText}`,
  )
...
async function run() {
  ...
  const patient = await loginUser(
    "patient@medislot.com",
    "patient123",
    "patient",
  )

  const doctor = await loginUser(
    "amitsharma@medislot.com",
    "doctor123",
    "doctor",
  )
  ...
  assert.strictEqual(
    bookRes.status,
    201,
    `Booking failed: ${bookData.message || bookData.error}`,
  )
  ...
  assert(
    patBookingNotif,
    "Patient received notification for booked appointment",
  )

  assert.strictEqual(patBookingNotif.title, "Appointment Booked")

  assert(
    patBookingNotif.message.includes(
      "Your appointment with Dr. Amit Sharma has been booked successfully.",
    ),
  )
  ...
  assert(
    docBookingNotif.message.includes(
  ...
  const adminBookingNotif = adminNotifsData.notifications.find(
    (n) =>
      (n.relatedId === appt1.id || n.relatedId === appt1.appointmentId) &&
  ...
  assert.strictEqual(
    adminBookingNotif.message,
    "A new appointment has been booked.",
  )

  pass(
    "TEST 1: Patient books appointment -> Patient, Doctor, Admin all received correct notifications.",
  )
  ...
  const confirmRes = await fetch(
    `${BASE_URL}/api/appointments/${appt1.id}/status`,
    {
      method: "PATCH",
  ...
  assert.strictEqual(
    confirmRes.status,
    200,
    `Doctor confirmation failed: ${confirmData.error}`,
  )
  ...
  assert(
    patConfirmNotif,
    "Patient received 'Appointment Confirmed' notification",
  )

  assert.strictEqual(patConfirmNotif.title, "Appointment Confirmed")

  assert(
    patConfirmNotif.message.includes(
      "Your appointment with Dr. Amit Sharma has been confirmed.",
    ),
  )

  pass(
    "TEST 2: Doctor confirms appointment -> Patient received 'Appointment Confirmed' notification.",
  )
  ...
  const docCancelRes = await fetch(
    `${BASE_URL}/api/appointments/${appt1.id}/status`,
    {
      method: "PATCH",
  ...
  assert(
    patCancelNotif,
    "Patient received 'Appointment Cancelled' notification from doctor cancellation",
  )

  assert.strictEqual(patCancelNotif.title, "Appointment Cancelled")

  assert(
    patCancelNotif.message.includes(
      "Your appointment with Dr. Amit Sharma has been cancelled.",
    ),
  )

  pass(
    "TEST 3: Doctor cancels appointment -> Patient received 'Appointment Cancelled'.",
  )
  ...
  const patCancelRes = await fetch(
    `${BASE_URL}/api/appointments/${appt2.id}/cancel`,
    {
      method: "PATCH",
  ...
  assert.strictEqual(
    patCancelRes.status,
    200,
    "Patient cancel request succeeded",
  )
  ...
  const docCancelledNotif = docNotifsAfterPatCancel.notifications.find(
    (n) =>
      n.type === "appointment_cancelled" &&
      (n.relatedId === appt2.id || n.relatedId === appt2.appointmentId),
  )

  assert(
    docCancelledNotif,
    "Doctor received 'Appointment Cancelled' when patient cancelled",
  )
  ...
  assert(
    adminCancelledNotif,
    "Admin received 'Appointment Cancelled' when appointment was cancelled",
  )

  assert.strictEqual(adminCancelledNotif.title, "Appointment Cancelled")

  assert.strictEqual(
    adminCancelledNotif.message,
    "An appointment has been cancelled.",
  )

  pass(
    "TEST 4: Patient cancels appointment -> Doctor and Admin both received 'Appointment Cancelled'.",
  )
  ...
  const completeRes = await fetch(
    `${BASE_URL}/api/appointments/${appt3.id}/status`,
    {
      method: "PATCH",
  ...
  assert.strictEqual(
    completeRes.status,
    200,
    "Complete status update succeeded",
  )
  ...
  const patCompleteNotif = patNotifsAfterComplete.notifications.find(
    (n) =>
      ...
      (n.relatedId === appt3.id || n.relatedId === appt3.appointmentId),
  )

  assert(
    patCompleteNotif,
    "Patient received 'Appointment Completed' notification",
  )

  assert.strictEqual(patCompleteNotif.title, "Appointment Completed")

  assert(
    patCompleteNotif.message.includes(
      "Your appointment with Dr. Amit Sharma has been marked as completed.",
    ),
  )

  pass(
    "TEST 5: Appointment completed -> Patient received 'Appointment Completed'.",
  )
  ...
  assert(
    newPatientNotif,
    "Admin received 'New Patient Registered' notification",
  )

  assert.strictEqual(newPatientNotif.title, "New Patient Registered")

  assert.strictEqual(
    newPatientNotif.message,
    "A new patient has registered in MediSlot.",
  )

  pass(
    "TEST 6: New patient registers -> Admin received 'New Patient Registered'.",
  )
  ...
  const newDocNotif = adminNotifsAfterDoc.notifications.find(
    (n) =>
      n.type === "doctor_added" &&
      (n.relatedId === addDocData.doctor.id ||
        n.relatedId === addDocData.doctor._id),
  )
  ...
  assert.strictEqual(
    newDocNotif.message,
    "Dr. Suresh Reddy has been added to MediSlot.",
  )
  ...
  assert(
    ...
    `Unread count > 0 (found ${countData.count})`,
  )
  ...
  const markOneRes = await fetch(
    `${BASE_URL}/api/notifications/${unreadNotif._id}/read`,
    {
      method: "PUT",
  ...
  const countAfterOne = await (
    await fetch(`${BASE_URL}/api/notifications/unread-count`, {
      headers: { Cookie: patient.cookie },
    })
  )
  ...
  assert.strictEqual(
    countAfterOne.count,
    baselineCount - 1,
    `Count decreased from ${baselineCount} to ${countAfterOne.count}`,
  )
  ...
  assert.strictEqual(
    countAfterAll.count,
    0,
    "Unread count became 0 after mark-all-read",
  )
  ...
  const rePatient = await loginUser(
    "patient@medislot.com",
    "patient123",
    "patient",
  )
  ...
  const reNotifs = await (
    await fetch(`${BASE_URL}/api/notifications`, {
      headers: { Cookie: rePatient.cookie },
    })
  )
  ...
  assert(
    reNotifs.notifications.length > 0,
    "User sees their persistent notification history in MongoDB",
  )
  ...
  for (const n of reNotifs.notifications) {
    assert.strictEqual(
      n.recipientId,
      rePatient.user.id || rePatient.user._id,
      "Every notification belongs to recipient",
    )
  ...
  pass(
    "TEST 11: Re-login verified persistent history strictly belonging to the authenticated user.",
  )
  ...
  assert.strictEqual(
    hackAttempt.status,
    404,
    "Cross-user notification modification rejected with 404",
  )
  ...
  assert.strictEqual(
    ...
    401,
    "Unauthenticated notification fetch rejected with 401",
  )

  // Cross-user deletion attempt

  const deleteHack = await fetch(
    `${BASE_URL}/api/notifications/${unreadNotif._id}`,
    {
      method: "DELETE",
  ...
  assert.strictEqual(
    deleteHack.status,
    404,
    "Cross-user notification deletion rejected with 404",
  )

  pass(
    "TEST 12: Security verified -> Cross-user access and unauthenticated requests are strictly rejected.",
  )
